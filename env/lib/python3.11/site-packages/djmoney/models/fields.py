...
class MoneyFieldProxy:
    ...
    def _money_from_obj(self, obj):
        ...
        return Money(
            amount=amount, currency=currency, decimal_places=self.field.decimal_places
        )
    ...
    def setup_default(self, default, default_currency, nullable):
        if default in (None, NOT_PROVIDED) or isinstance(default, Money):
            ...
            try:
                ...
                amount, currency = (
                    default.decode() if isinstance(default, bytes) else default
                ).split(" ", 1)
            except ValueError:
            ...
            raise ValueError(
                f"default value must be an instance of Money, is: {default}"
            )
